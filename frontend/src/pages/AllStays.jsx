...
export default function HotelDetails() {
  ...
  // YE RAHA SAARA 16 HOTELS KA DATA
  const localHotels = {
    ...
    "local_05": { name: "Dhruvnanda Homestay", location: "Athali", price: 1500, image: "https://images.unsplash.com/photo-1499793983690-e29da59ef1c2?w=800" },
  ...
  const handlePayment = async (amount) => {
    try {
      const { data } = await axios.post(`${BACKEND_URL}/api/payment/create-order`, { amount });
      const options = {
        ...
        name: "The Himalayans",
        order_id: data.id,
        handler: () => alert("🎉 Booking Successful!"),
        theme: { color: "#006ce4" }
      };
  ...
  if (!hotel) return <div style={{padding:"50px"}}>Loading Details...</div>;
  ...
  return (
    <div style={{ padding: "40px", maxWidth: "800px", margin: "auto" }}>
      ...
      <h2 style={{color:"#006ce4"}}>Price: ₹{hotel.price} per night</h2>
      <button onClick={() => handlePayment(hotel.price)} style={{padding:"15px 30px", background:"#006ce4", color:"white", border:"none", borderRadius:"8px", cursor:"pointer", fontSize:"16px"}}>
        Reserve Your Stay
      </button>
    </div>
  );
